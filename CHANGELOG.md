...
- **工具桥接提示词严格化（三协议一致）**：OpenAI / Claude / Gemini 的桥接提示词均明确声明上游模型**没有原生工具调用能力**、工具由本服务执行，必须**按约定 JSON 格式输出**（不得用 markdown 代码围栏、不得在 JSON 前后附加文字），并说明**结果会自动回传**、无需请求许可、也不存在其他获取途径；同时新增「不得凭自身知识或网页自带卡片作答」与「不得凭空编造工具结果」两条约束。repair 提示词同步加入同一前提说明。
...
- **工具参数静默降级**：`normalizeArguments` 解析失败不再静默替换为 `{}`，改为返回错误并走 repair 轮。此前空参数会让工具被错误执行，而客户端看到的是成功调用（仅 `""`/`null` 仍合法映射为 `{}`）。
- **工具 payload 诱饵对象**：解析从「贪心取第一个平衡 JSON 对象」改为「扫描全部顶层对象、取第一个通过校验者」，前置的示例对象不再遮蔽有效工具调用；截断对象直接跳过，不猜缺失参数。
- **工具桥接复用判据**：轻量提示词（复用同一 Gemini 会话已下发的工具定义）原先只判断「有 provider 会话 ID 且 AutoContext」，而显式传入 `conversation_id` 会让首轮即为 `AutoContext=true`，导致首轮只发用户原话、模型不知道有哪些工具与参数（上游 `prompt_len` 仅 53）。现改为必须由 `toolBridgeContextReady` 确认该会话已收到**同一份**工具集（签名比对）才走轻量路径；工具集变更同样强制重发完整定义。此修复让此前零调用的 `toolBridgeContextReady` 真正生效。
- **工具桥接复用路径丢失协议约束**：轻量路径在 `tool_choice=auto` 下原本只发用户原话，模型可能凭自身知识作答。现改为始终附带紧凑版严格契约（仍不重发工具清单），保证「无原生工具调用、按 JSON 格式输出、结果自动回传」的说明在每轮都在场。
- **删除死代码**：`buildToolBridgeContinuationPrompt` 自 `926ed5c` 起已无任何调用点，直接移除。
